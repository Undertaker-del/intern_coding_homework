...
def main() -> None:
    ...
    step("モデル比較(Ridge/LGBM/MLP/DLinear)", ["src/model_compare.py"])
    step("特徴量数の感度", ["src/feature_sensitivity.py"])
    step("予測区間の較正(conformal)", ["src/conformal.py"])
    step("結果サマリ図", ["src/report_figures.py"])
    step("追加検証の図", ["src/extra_figures.py"])
