...
from config import (FIGURES_DIR, HORIZONS_HOURLY, HORIZONS_MINUTE, LOAD_COLS,
                    ...
                    TARGET)
from evaluate import mae, rmse, skill, threshold_alarm_metrics
...
def run_horizon(df: pd.DataFrame, horizon: int, period: int) -> dict:
    ...
    def fit_delta_lgbm(X, y, dates):
        ...
        m = models.LightGBMModel().fit(Xtr, dtr, eval_set=[(Xva, dva)])
    ...
    # seasonal naive: ŷ(t+h)=OT(t+h-period)。元系列 OT を period 前から取得
    ...
    seas_pos = target_pos - period
    valid_seas = seas_pos >= 0
    seas_pred_full = np.full(len(base_time), np.nan)
    seas_pred_full[valid_seas] = ot[seas_pos[valid_seas]]
    seas_te = seas_pred_full[te_mask.to_numpy()]
